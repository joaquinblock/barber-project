...
describe('CreateBarbershopDto', () => {
  const validateDto = async (dto: any) => {
    const instance = plainToInstance(CreateBarbershopDto, dto);
  ...
  it('debería validar una Barbería válida con todos los campos', async () => {
    const dto = {
  ...
  it('debería aplicar Trim al nombre y dirección', async () => {
    ...
    const instance = plainToInstance(CreateBarbershopDto, dto);
    expect(instance.name).toBe('Barbería VIP');
